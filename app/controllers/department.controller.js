...
const defaultModel = require('../models/department.model');
const usersAuth = require('../models/user.model');
const utilities = require('../helpers/utilities.helper');
const { createSchema, updateSchema } = require('../helpers/validations/department.schema');
...
exports.update = async (req, res) => {
    try {
        ...
        if (!driverDetail) {
            res.status(400).send({
                status: 'error',
                message: lang.t('department.err.read')
            });
        }
...
exports.search = async (req, res) => {
    try {
        ...
        SELECT COUNT(*) as total FROM departments WHERE status = '${STATUS_ACTIVE}'`);
