...
exports.update = async (req, res) => {
    try {
        ...
        if (!defaultVariable) {
            res.status(400).send({
                status: 'error',
                message: lang.t('vehicle.err.read')
            });
        }

        const updatedProfitCenter = await defaultModel.update(defaultVariable._id, body)
...
exports.search = async (req, res) => {
    try {
        ...
        SELECT COUNT(*) as total FROM vehicle_details WHERE status = '${STATUS_ACTIVE}'`);
        ...
        return res.status(200).send({
            status: 'success',
            message: lang.t('vehicle.suc.search'),
            data: data,
            pagination: {
                page_num: pageNum,
